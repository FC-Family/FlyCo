...
int main(int argc, char **argv) {
    ...
    auto x_t1 = std::chrono::high_resolution_clock::now();

    float3 result_cpu[sample_number];
    int bucketSize = 10 << kd_high;
    ...
    auto x_t2 = std::chrono::high_resolution_clock::now();
    std::chrono::duration<double, std::milli> x_ms = x_t2 - x_t1;
    std::cout << "Total time: " << x_ms.count() << "ms" << std::endl;
