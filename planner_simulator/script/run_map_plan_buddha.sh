...
tmux send-keys -t map_plan:0.3 "source $SCRIPT_DIR/../src/3rdparty/embree-3/embree.zsh && source $SCRIPT_DIR/../devel/setup.zsh && stdbuf -oL roslaunch flyco_planner_manager flyco_planner_buddha.launch 2>&1 | tee -a $LOG_FILE; exec zsh" C-m
sleep 1s 
