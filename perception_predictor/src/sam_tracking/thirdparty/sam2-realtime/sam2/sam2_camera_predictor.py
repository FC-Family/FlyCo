...
class SAM2CameraPredictor(SAM2Base):
    ...
    def add_new_promot_during_track(
        ...
        """Insert a new prompt on the latest frame during tracking (real-time behavior).

        Constraints:
        - Must be called after tracking has started.
        - Adding a brand-new target mid-tracking is explicitly unsupported (NotImplementedError).
        - Operates on the current/latest frame_idx = num_frames - 1.
        """
        assert (
            self.condition_state["tracking_has_started"] == True
        ), "Cannot add new points or mask during tracking without calling track()"

        # Disallow creating a brand-new object during tracking
        if if_new_target:
            raise NotImplementedError()

        # Target the latest frame
        frame_idx = self.condition_state["num_frames"] - 1

        # Default to last obj_id if not provided
        if obj_id is None:
            obj_id = self.condition_state["obj_ids"][-1]

        # Temporarily mark as not started so propagate_in_video_preflight can re-run
        self.condition_state["tracking_has_started"] = False
    ...
    def track(
        ...
        geo_score = current_out.get("geo_score", None)
        ...
        self._manage_memory_obj(self.frame_idx, current_out, add_memory=add_memory)

        _, video_res_masks = self._get_orig_video_res_output(pred_masks_gpu)
        return obj_ids, video_res_masks

    def _manage_memory_obj(self, frame_idx, current_out, add_memory=False):
        output_dict = self.condition_state["output_dict"]
        ...
        # print("len ",len(non_cond_frame_outputs))
        if len(non_cond_frame_outputs) > 6 * self.num_maskmem - len(
            self.add_memory_list
        ):
            for t in range(
                0,
                len(non_cond_frame_outputs)
                - 6 * self.num_maskmem
                + len(self.add_memory_list),
            ):
        ...
        #     "len  non_cond_frame_outputs",
        #     len(non_cond_frame_outputs),
        #     "len  cond_frame_outputs",
        #     len(output_dict["cond_frame_outputs"]),
        #     " len memoy add_memory_list ",
        #     len(self.add_memory_list),
        # )
