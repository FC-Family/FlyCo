...
class SAM2Base(torch.nn.Module):
    def __init__(
        ...
        # depth-guided scoring
        depth_score_weight: float = 0.0,
    ):
        super().__init__()
        ...
        self.memory_bank_geo_score_threshold = memory_bank_geo_score_threshold
        self.depth_score_weight = depth_score_weight
    ...
    def _forward_sam_heads(
        ...
        # Optional depth prior scoring (projected depth mask vs. predicted mask)
        selection_scores = ious
        depth_scores = None
        if depth_masks is not None and self.depth_score_weight > 0:
            depth_masks = depth_masks.to(device)
            if depth_masks.dim() == 3:
                depth_masks = depth_masks.unsqueeze(1)
            depth_masks_bool = (
                depth_masks if depth_masks.dtype == torch.bool else depth_masks > 0
            )
            if depth_masks_bool.shape[-2:] != high_res_multimasks.shape[-2:]:
                depth_masks_bool = F.interpolate(
                    depth_masks_bool.float(),
                    size=high_res_multimasks.shape[-2:],
                    mode="nearest",
                ) > 0.5
            pred_binary = high_res_multimasks > 0.0
            intersection = (
                (pred_binary & depth_masks_bool).flatten(2).sum(-1).float()
            )
            union = (pred_binary | depth_masks_bool).flatten(2).sum(-1).float()
            ...
            selection_scores = (
                (1 - self.depth_score_weight) * selection_scores.float()
                + self.depth_score_weight * depth_scores
            )
        ...
        if multimask_output and self.samurai_mode:
            if self.kf_mean is None and self.kf_covariance is None or self.stable_frames == 0:
                best_iou_inds = torch.argmax(selection_scores, dim=-1)
                ...
                # weighted score combining motion (kf_ious) and model/depth scoring
                weighted_scores = self.kf_score_weight * kf_ious + (1 - self.kf_score_weight) * selection_scores
                best_iou_inds = torch.argmax(weighted_scores, dim=-1)
                batch_inds = torch.arange(B, device=device)
                low_res_masks = low_res_multimasks[batch_inds, best_iou_inds].unsqueeze(1)
                high_res_masks = high_res_multimasks[batch_inds, best_iou_inds].unsqueeze(1)
                if sam_output_tokens.size(1) > 1:
                ...
                if False:
                    ...
                    self.history[self.frame_cnt] = {
                        ...
                        "kf_ious": kf_ious,
                        "weighted_scores": weighted_scores.cpu(),
        ...
        selected_geo_score = None
        if depth_scores is not None:
            if torch.is_tensor(best_iou_inds):
                batch_inds = torch.arange(B, device=device)
                selected_geo_score = depth_scores[batch_inds, best_iou_inds]
            else:
                selected_geo_score = depth_scores[:, best_iou_inds]
    ...
    def _prepare_memory_conditioned_features(
        ...
        if not is_init_cond_frame:
            ...
            if add_memory:
                ...
                self.add_memory_list.append(prev_frame_idx)
                if len(self.add_memory_list) > self.num_maskmem - 2:
                    self.add_memory_list.pop(0)

            for frame_idex in self.add_memory_list:
                out = output_dict["non_cond_frame_outputs"].get(frame_idex, None)
                if out is not None:
                    t_pos_and_prevs.append((len(t_pos_and_prevs), out))
            ...
            if self.samurai_mode:
                ...
                if frame_idx > 1:  # Ensure we have previous frames to evaluate
                    ...
                    idex_keys = list(output_dict['non_cond_frame_outputs'].keys())
                    for i in idex_keys:  # Iterate backwards through previous frames
                        # print(f"Frame index: {i}")
                        # print(f"Output dict: ",len(output_dict['non_cond_frame_outputs']))
                        ...
                        geo_score = output_dict["non_cond_frame_outputs"][i].get("geo_score", None)  # Get depth/geometry alignment score if available
                        ...
                        if iou_score.item() > self.memory_bank_iou_threshold and \
                           ...
                           (geo_score is None or self.memory_bank_geo_score_threshold <= 0 or geo_score.item() > self.memory_bank_geo_score_threshold):
                ...
                for t_pos in range(len(t_pos_and_prevs), self.num_maskmem):  # Iterate over the number of mask memories
                ...
                for t_pos in range(len(t_pos_and_prevs), self.num_maskmem):
                    t_rel = self.num_maskmem - t_pos  # how many frames before current frame
                    if t_rel == len(t_pos_and_prevs):
                    ...
                    # print(f"Frame index: {prev_frame_idx}",)
                    if out is None:
    ...
    def track_step(
        ...
        # print("track_step ",self.maskmem_tpos_enc)
        ...
        current_out["geo_score"] = geo_score
